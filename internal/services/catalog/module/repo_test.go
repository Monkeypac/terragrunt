...
func TestFindModules(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.repoPath, func(t *testing.T) {
			...
			require.NoError(t, os.Rename(filepath.Join(tc.repoPath, "gitdir"), filepath.Join(tc.repoPath, ".git")))
			defer require.NoError(t, os.Rename(filepath.Join(tc.repoPath, ".git"), filepath.Join(tc.repoPath, "gitdir")))
